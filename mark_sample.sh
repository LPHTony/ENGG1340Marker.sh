        ...
        do
            ...
            then
                Number="$(ls ${Path}/"${file}" | grep -Po '\d+_\d+')"
                    ...
                    if diff myoutput"${Number}".txt ${Path}/output"${Number}".txt --strip-trailing-cr 1>/dev/null 2>&1
                    then
                        echo Sample Testcase "${Number}" Passed
                        rm myoutput"${Number}".txt
                    else
                        echo Sample Testcase "${Number}" Failed
